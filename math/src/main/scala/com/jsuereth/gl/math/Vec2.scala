...
class Vec2[T : ClassTag](private[this] val values: Array[T]) {
  ...
  def +(other: Vec2[T])(using Numeric[T]): Vec2[T] = Vec2(x+other.x, y+other.y)
  def -(other: Vec2[T])(using Numeric[T]): Vec2[T] = Vec2(x-other.x, y-other.y)
  // behaves as GLSL would, just multiplies pairs...
  def *(other: Vec2[T])(using Numeric[T]): Vec2[T] = Vec2(x*other.x, y*other.y)

  /** The dot product of this vector and another. */
  def dot(other: Vec2[T])(using Numeric[T]): T = x*other.x + y*other.y
  ...
  def normalize(using Fractional[T], Rootable[T]): Vec2[T] =
